    ...
    for (c of cards) {
        $('#card-list').append(
            `<div id="${c.id}" class="vtes-card" data-release='${c.release_date}' data-name="${c.name}" data-cardtype='${c.type[0]}' data-favorite='2000-01-01'>
                    <table><tbody>
                        <tr><td>
                            <a href="${c.link}?affiliate_id=2900918" target="_blank"><img class="pod-image" src="https://statics.bloodlibrary.info/img/sets/pod/${c.id}.jpg"/></a>
                        </td></tr>
                        <tr><td class="vtes-card-name red-text">
                            <a class="favorite-icon" onclick="toggleFavorite(this,${c.id})" >${localStorage.getItem(c.id) === null ? empty_star_icon : full_star_icon}</a> <a href="${c.link}?affiliate_id=2900918" target="_blank">${c.name}</a>
                        </td></tr>
                    </tbody></table>
            </div>`
        );
